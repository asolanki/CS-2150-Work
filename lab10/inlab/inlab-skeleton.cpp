...
void parse( huffmanTree *tree, string target ) {
    ...
    while ( target.length() > 0 )
    {
        ...
        while (curr->left || curr->right)
        {
           if ( target[0] == '0' )
           {
     //          cout << "0 found, go left" << endl;
     //         cout <<  "(target = " << target << ")" << endl;
               curr = curr->left;
               target = target.substr( 1, target.length() - 1 );
     //          cout << "target now: " << target << endl;
           }
           else if ( target[0] == '1' )
           {
     //          cout << "1 found, go right" << endl;
     //          cout <<  "(target = " << target << ")" << endl;
               curr = curr->right;
               target = target.substr( 1, target.length() - 1 );
     //          cout << "target now: " << target << endl;
     ...
     //   cout << "leaf found! char: " << curr->elem << endl;
...
int main (int argc, char **argv) {
  ...
  while ( true ) {
    ...
    if ( buffstring[0] == '0' )
    {
        cout << "created new node at " << buffer << " for char " << first << endl;
