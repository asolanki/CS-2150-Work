...
{
    // YOUR CODE HERE
    double d = (int_path_length(root, 0)/(double)num_nodes);
    return d;
}
...
int AvlTree::int_path_length(AvlNode *t, int depth)
{
    ...
    /* If it is called on the root node, then the depth would be zero (and the function 
     * would return the total internal path length of the entire tree). The root node will
     * call it recursively (with depth=1) on it's children. If it is called on a leaf node,
     * then the depth of that particular node is just the value of the depth parameter. If if 
     * is called on an internal node, then it is going to call itself on each of the children
     * (in each case yielding the internal path length for the child sub-trees), plus the depth
     * of that particular internal node.
     */
